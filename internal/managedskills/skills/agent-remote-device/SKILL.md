...
- A repeated named `observe` retains that application's exact last process/window binding. Use an unnamed observation to follow the eligible frontmost application, or a window-changing action and its returned state to switch windows deliberately. When an application has multiple unbound windows or signed instances, make the intended window frontmost before the first named observation; the device fails closed rather than choosing a background window.
