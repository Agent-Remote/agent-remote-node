...
- 来自 `Agent-Remote/agent-remote-ego-browser` release `0.1.16` 的 Linux
  `ego-browser` wrapper；
...
wrapper 安装完成本身不代表端到端 capability 可以开启。旧 `0.1.15` release evidence 不能
认证 `0.1.16` release；只有 `0.1.16` 经固定 workflow 发布、root evidence 记录准确的
